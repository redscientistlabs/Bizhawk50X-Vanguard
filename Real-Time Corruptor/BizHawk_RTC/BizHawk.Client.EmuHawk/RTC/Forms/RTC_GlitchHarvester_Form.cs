...
namespace RTC
{
    public partial class RTC_GlitchHarvester_Form : Form
    {
		...
		public void RemoveSelected(bool force = false)
		{
			...
			if ((dgvStockpile.SelectedRows.Count != 0 && (MessageBox.Show("Are you sure you want to remove the selected stockpile entries?", "Delete Stockpile Entry?", MessageBoxButtons.YesNo) == DialogResult.Yes)) || Control.ModifierKeys == Keys.Control)
					foreach (DataGridViewRow row in dgvStockpile.SelectedRows)
