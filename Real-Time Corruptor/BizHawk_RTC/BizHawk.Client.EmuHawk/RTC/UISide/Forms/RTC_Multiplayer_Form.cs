...
namespace RTC
{
	public partial class RTC_Multiplayer_Form : Form, IAutoColorize
	{
		...
		private void RTC_Multi_Form_Load(object sender, EventArgs e)
		{
			...
			NetCoreImplementation.Multiplayer = new RTC_NetCore();

			NetCoreImplementation.Multiplayer.ClientConnecting += Multiplayer_ClientConnecting;
			NetCoreImplementation.Multiplayer.ClientConnected += Multiplayer_ClientConnected;
			NetCoreImplementation.Multiplayer.ClientDisconnected += Multiplayer_ClientDisconnected;
			NetCoreImplementation.Multiplayer.ClientConnectionLost += Multiplayer_ClientConnectionLost;
			NetCoreImplementation.Multiplayer.ClientReconnecting += Multiplayer_ClientReconnecting;

			NetCoreImplementation.Multiplayer.ServerStarted += Multiplayer_ServerStarted;
			NetCoreImplementation.Multiplayer.ServerConnected += Multiplayer_ServerConnected;
			NetCoreImplementation.Multiplayer.ServerDisconnected += Multiplayer_ServerDisconnected;
			NetCoreImplementation.Multiplayer.ServerConnectionLost += Multiplayer_ServerConnectionLost;
		...
		private void btnStartClient_Click(object sender, EventArgs e)
		{
			...
			{
				NetCoreImplementation.Multiplayer.StopNetworking();
				return;
			}

			NetCoreImplementation.Multiplayer.StartNetworking(NetworkSide.CLIENT);
		}
		...
		private void btnStartServer_Click(object sender, EventArgs e)
		{
			...
			NetCoreImplementation.Multiplayer.StartNetworking(NetworkSide.SERVER);
		}

		private void btnPushBlastToServer_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendBlastlayer();
		}
		...
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLROM), false);
		}

		private void btnPushGameToServer_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLROM), true);
		}

		private void btnPullStateFromServer_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLSTATE), false);
		}

		private void btnPushStateToServer_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLSTATE), true);
		}

		private void tbServerPort_TextChanged(object sender, EventArgs e)
		{
			if (NetCoreImplementation.Multiplayer.side == NetworkSide.DISCONNECTED)
				NetCoreImplementation.Multiplayer.port = Convert.ToInt32(tbServerPort.Text);
			else
				tbServerPort.Text = NetCoreImplementation.Multiplayer.port.ToString();
		}

		private void btnSwapGameState_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SwapGameState();
		}

		private void btnPushScreenToPear_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLSCREEN), true);
		}

		private void btnPullScreenToPear_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.PULLSCREEN), false);
		}
		...
		private void StreamTimer_Tick(object sender, EventArgs e)
		{
			if (RTC_Hooks.BIZHAWK_ISMAINFORMVISIBLE())
			{
				...
				if (cbCompressStream.Checked)
					...
					cmdBack.screen = bmp;
				NetCoreImplementation.Multiplayer.PeerCommandQueue.AddLast(cmdBack);
			}
		}
		...
		{
			NetCoreImplementation.Multiplayer.ClearNetowrkCache();
		}
		...
		public void StopGameOfSwap(bool fromStopCommand = false)
		{
			...
			if (!fromStopCommand)
				NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.GAMEOFSWAPSTOP), false);
		}
		...
		public void StartGameOfSwap(bool isGameHost)
		{
			...
			if (isGameHost)
				NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.GAMEOFSWAPSTART), false);
		...
		{
			NetCoreImplementation.Multiplayer.SendStashkey();
		}

		private void btnRequestStream_Click(object sender, EventArgs e)
		{
			NetCoreImplementation.Multiplayer.SendCommand(new RTC_Command(CommandType.REQUESTSTREAM), false);
		}
		...
		private void btnStartServer_MouseDown(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Right)
			{
				...
				(columnsMenu.Items.Add("BRUTAL DISCONNECT", null, new EventHandler((ob, ev) => { NetCoreImplementation.Multiplayer.StopNetworking(true, false); })) as ToolStripMenuItem).Enabled = NetCoreImplementation.Multiplayer.side != NetworkSide.DISCONNECTED;
		...
		{
			NetCoreImplementation.Multiplayer.address = tbClientAdress.Text;
		}
