...
namespace RTC
{
	public partial class RTC_VmdAct_Form : ComponentForm, IAutoColorize
	{
		...
		public bool ComputeActiveTableActivity()
		{
			...
			//	long domainSize = (long)NetCoreImplementation.SendCommandToBizhawk(new RTC_Command(CommandType.REMOTE_DOMAIN_GETSIZE) { objectValue = cbSelectedMemoryDomain.SelectedItem.ToString()}, true);
		...
		private void btnActiveTableAddDump_Click(object sender, EventArgs e)
		{
			...
			NetCoreImplementation.SendCommandToBizhawk(new RTC_Command(CommandType.REMOTE_DOMAIN_ACTIVETABLE_MAKEDUMP)
			{
