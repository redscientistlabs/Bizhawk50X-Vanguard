...
namespace RTC
{
	public static class RTC_Core
	{
		...
		{
			get => (bool)RTC_Unispec.RTCSpec[RTCSPEC.CORE_REROLLADDRESS.ToString()];
			set => RTC_Unispec.RTCSpec.Update(RTCSPEC.CORE_REROLLADDRESS.ToString(), value);
		}
		public static bool RerollSourceAddress
		{
			get => (bool)RTC_Unispec.RTCSpec[RTCSPEC.CORE_REROLLSOURCEADDRESS.ToString()];
			set => RTC_Unispec.RTCSpec.Update(RTCSPEC.CORE_REROLLSOURCEADDRESS.ToString(), value);
		}
		...
		public static BlastLayer Blast(BlastLayer _layer, string[] _selectedDomains)
		{
			...
			try
			{
				...
				else
				{
					...
					int _Intensity = RTC_Core.Intensity; //general RTC intensity

					if ((RTC_Core.SelectedEngine == CorruptionEngine.HELLGENIE ||
					...
					switch (RTC_Core.Radius) //Algorithm branching
					{
