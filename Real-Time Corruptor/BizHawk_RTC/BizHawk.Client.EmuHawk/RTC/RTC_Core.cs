...
namespace RTC
{
	public static class RTC_Core
	{
		...
		public static void CloseAllRtcForms() //This allows every form to get closed to prevent RTC from hanging
		{
			...
			if (!RTC_Hooks.isRemoteRTC && !(bool)RTCSpec[Spec.CORE_DONTCLEANSAVESTATESONQUIT.ToString()])
			{
		...
		public static BlastUnit GetBlastUnit(string _domain, long _address, int precision)
		{
			...
			switch ((CorruptionEngine)(RTC_Unispec.RTCSpec[Spec.CORE_SELECTEDENGINE.ToString()]))
			{
		...
		public static BlastLayer Blast(BlastLayer _layer, string[] _selectedDomains)
		{
			...
			try
			{
				...
				else if ((CorruptionEngine)RTCSpec[Spec.CORE_SELECTEDENGINE.ToString()] == CorruptionEngine.BLASTGENERATORENGINE)
				{
				...
				else
				{
					...
					int _Intensity = (int)(RTCSpec[Spec.CORE_INTENSITY.ToString()] ?? -1); //general RTC intensity

					if (((CorruptionEngine)RTCSpec[Spec.CORE_SELECTEDENGINE.ToString()] == CorruptionEngine.HELLGENIE || (CorruptionEngine)RTCSpec[Spec.CORE_SELECTEDENGINE.ToString()] == CorruptionEngine.FREEZE || (CorruptionEngine)RTCSpec[Spec.CORE_SELECTEDENGINE.ToString()] == CorruptionEngine.PIPE) && _Intensity > (int)RTCSpec[Spec.STEP_MAXINFINITEBLASTUNITS.ToString()])
						_Intensity = (int)RTCSpec[Spec.STEP_MAXINFINITEBLASTUNITS.ToString()]; //Capping for cheat max

					switch ((BlastRadius)RTC_Unispec.RTCSpec[Spec.CORE_RADIUS.ToString()]) //Algorithm branching
					{
						case BlastRadius.SPREAD: //Randomly spreads all corruption bytes to all selected domains
							...
							for (int i = 0; i < _Intensity; i++)
							{
								...
								bu = GetBlastUnit(Domain, RandomAddress, (int)RTCSpec[Spec.CORE_CURRENTPRECISION.ToString()]);
								if (bu != null)
									bl.Layer.Add(bu);
							}
		...
		public static BlastTarget GetBlastTarget()
		{
			...
			string[] _selectedDomains = (string[])RTCSpec[Spec.MEMORYDOMAINS_SELECTEDDOMAINS.ToString()];
		...
		public static void LoadDefaultRom()
		{
			...
			int lastLoaderRom = (int)(RTCSpec[Spec.CORE_LASTLOADERROM.ToString()] ?? -1);
			int newNumber = (int)(RTCSpec[Spec.CORE_LASTLOADERROM.ToString()] ?? -1);
