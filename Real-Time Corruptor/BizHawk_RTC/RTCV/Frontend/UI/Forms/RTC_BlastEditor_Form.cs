...
namespace RTC
{
	public partial class RTC_BlastEditor_Form : Form, IAutoColorize
	{
		...
		private void btnDisable50_Click(object sender, EventArgs e)
		{
			...
			foreach (BlastUnit bu in sk.BlastLayer.Layer.OrderBy(x => RTC_CorruptCore.RND.Next()).Take(sk.BlastLayer.Layer.Count / 2))
			{
				if (!(bool)bu2RowDico[bu].Cells["dgvBlastUnitLocked"].Value)
				{
		...
		private void btnSendToStash_Click(object sender, EventArgs e)
		{
			...
			StashKey newSk = (StashKey)sk.Clone();
			//newSk.Key = RTC_CorruptCore.GetRandomKey();
			//newSk.Alias = null;
		...
		private void PopulateAddressContextMenu(int column, int row)
		{
...

			{
				...
				{
					NetCoreImplementation.SendCommandToRTC(new RTC_Command(CommandType.BIZHAWK_OPEN_HEXEDITOR_ADDRESS) { objectValue = new object[] { domain, address } });
				}))).Enabled = true;
			}
		...
		private void replaceSavestateFromFileToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			//Get a new key
			sk.ParentKey = RTC_CorruptCore.GetRandomKey();
			...
			StashKey temp = new StashKey(RTC_CorruptCore.GetRandomKey(), sk.ParentKey, sk.BlastLayer);
			sk.SyncSettings = temp.SyncSettings;
		}
